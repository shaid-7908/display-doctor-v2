...
class TechnicianController {
  ...
  getAssignedIssues = asyncHandler(async (req, res) => {
    ...
    fromDate =
      fromDate && String(fromDate).trim() !== "" ? fromDate : undefined;
  ...
  generateIssueReport = asyncHandler(async (req, res) => {
    ...
    const exixtingReport = await IssueReportModel.findOne({
      issue_id: issue_id,
    });
    if (exixtingReport) {
      return sendError(
        res,
        "Issue report already created",
        null,
  ...
  getIssueReportList = asyncHandler(async (req, res) => {
    ...
    return sendSuccess(
      res,
      "Issue report fetched successfully",
      issueReports,
      STATUS_CODES.OK
    );
  });
  renderIssueReportPage = asyncHandler(async (req, res) => {
    res.render("technician-issue-report-page", { default_user: req.user });
  });
}
